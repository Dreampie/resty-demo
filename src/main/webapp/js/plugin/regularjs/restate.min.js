(function (root, factory) {
  ...
  var restate = function (option) {
    ...
    stateman.state = function (name, Component, config) {
      if (!Component) {
        return preStae.call(stateman, name)
      }
      if (BaseComponent) {
        if (typeof Component === "string" || Array.isArray(Component)) {
          Component = BaseComponent.extend({template: Component})
        }
      ...
      if (Component.extend && Component.__after__) {
        ...
        var state = {
          component: null, enter: function (step) {
            var data = {$param: step.param}, component = this.component, noComponent = !component, view;
            if (noComponent) {
              component = this.component = new Component({data: data, $state: stateman})
            }
            ...
            if (parent.component) {
              ...
              if (!view) {
                throw this.parent.name + " should have a element with [ref=view]"
              }
            }
            ...
            component.$update({$param: option.param});
